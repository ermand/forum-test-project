...
async def read_posts(
    _request: Request, db: SessionDep, params: Annotated[PaginationParams, Query()]
):
...
async def read_single_post(
    _request: Request, id: Annotated[UUID, Path()], db: SessionDep
):
...
async def create_post(
    ...
    post = await post_service.create_new_post(
        db, post_data=post, user_id=current_user.id
    )
    return ApiResponse(data=post)
