...
async def test_create_comment(db, auth_user, test_posts):
    ...
    comment = CommentCreate(content="Test", post_id=post1.id)

    result = await comment_service.comment_create(
        db, comment_data=comment, user_id=auth_user.id
    )
...
async def test_get_comments_by_post_id(db, test_posts, test_comments):
    ...
    result = await comment_service.get_post_comments(
        db, post_id=post1.id, params=params
    )
...
async def test_update_comment(db, auth_user, test_posts, test_comments):
    ...
    result = await comment_service.update_post_comment(
        db, comment_data=comment, user_id=auth_user.id, comment_id=c2.id
    )
...
async def test_delete_comment(db, auth_user, test_comments):
    ...
    result = await comment_service.delete_post_comment(
        db, comment_id=c2.id, user_id=auth_user.id
    )
